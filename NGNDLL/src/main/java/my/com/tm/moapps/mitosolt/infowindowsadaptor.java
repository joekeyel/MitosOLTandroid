...
class infowindowsadaptor implements GoogleMap.InfoWindowAdapter {
    ...
    infowindowsadaptor() {
        if(livemap.getInstance()!= null) {
            mymarkerview = livemap.getInstance().getLayoutInflater().inflate(R.layout.infowindowlayout, null);
        }
    ...
    private void render(final Marker marker, final View view) {
        ...
        final StorageReference img2 = storageRef.child("remote_camera/Cabinet/"+imagename+".jpg");


//        final ImageView img = (ImageView)view.findViewById(R.id.imageView1);
//
//        Glide
//                .with(livemap.getInstance().getBaseContext())
//                .using(new FirebaseImageLoader())
//                .load(img2)
//                .into(img);


        storageRef.child("remote_camera"+ File.separator+imagename+".jpg").getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
